 ...
 *
 */
public class TimestampUserRatingProfileSplitterTest {
	@Test
	public void testRandomSplit() {
		UserRatingProfileSplitter splitter = new TimestampUserRatingProfileSplitter(0.333333);
		List<Rating> ratings = new ArrayList<Rating>();
		ratings.add(new SimpleRating(5, 2, 5, 42));
		ratings.add(new SimpleRating(5, 5, 5, 59));
		ratings.add(new SimpleRating(5, 3, 5, 37));
		...
		assertEquals(5, split.getQueryVector().get(2), 1.0e-6);
		assertEquals(5, split.getQueryVector().get(3), 1.0e-6);
		assertEquals(5, split.getProbeVector().get(5), 1.0e-6);
	}
}
